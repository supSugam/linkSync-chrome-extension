...
const SetUsername:React.FC<SetUsernameProps> = ({setErrorMessage,errorMessage,loading,setUsername}) => {
  return (
    <>
    <MdAlternateEmail size={24} className='absolute top-[10%] left-[3%] z-10 text-slate-400'/>
    ...
    <button type='submit' className='username__button relative overflow-hidden px-4 py-3 w-1/2 hover:bg-[#0b0a1f] bg-[#100e29] transition-all duration-150 outline-none rounded-xl text-xl font-semibold h-[50px] flex items-center justify-center'>
      {
        loading ? <ColorRing
        visible={true}
        height="45"
